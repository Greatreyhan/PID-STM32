...
void PID(PID_TypeDef *hPID, double *Input, double *Output, double *Setpoint, double Kp, double Ki, double Kd, PID_Propotional_TypeDef POn, PID_Control_Direction_TypeDef ControllerDirection)
{
	...
	hPID->InAuto     = (PID_Mode_TypeDef) 0;
...
{
	PID(hPID, Input, Output, Setpoint, Kp, Ki, Kd, PID_PROPOTIONAL_ON, ControllerDirection);
}
...
void            PID_SetMode(PID_TypeDef *hPID, PID_Mode_TypeDef Mode)
{
	...
	/* ~~~~~~~~~~ Initialize the PID ~~~~~~~~~~ */
	if (newAuto && !hPID->InAuto)
	{
		PID_Init(hPID);
	}
	
	hPID->InAuto = (PID_Mode_TypeDef)newAuto;
	
}
PID_Mode_TypeDef PID_GetMode(PID_TypeDef *hPID)
{
	return hPID->InAuto ? PID_AUTOMATIC_MODE : PID_MANUAL_MODE;
}
...
void PID_SetTunings2(PID_TypeDef *hPID, double Kp, double Ki, double Kd, PID_Propotional_TypeDef POn)
{
	...
	/* ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ */
	hPID->POn    = POn;
	hPID->POnE   = (PID_Propotional_TypeDef)(POn == PID_PROPOTIONAL_ON);
...
{
	/* ~~~~~~~~~~ Check parameters ~~~~~~~~~~ */
	if ((hPID->InAuto) && (Direction !=hPID->ControllerDirection))
	{
